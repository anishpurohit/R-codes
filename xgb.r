...
X_train$City_Category_A <-ifelse(X_train$City_Category == 'A',1,0)
X_train$City_Category_B <-ifelse(X_train$City_Category == 'B',1,0)
X_train <- X_train[,-c(6)]
X_test$City_Category_A <-ifelse(X_test$City_Category == 'A',1,0)
X_test$City_Category_B <-ifelse(X_test$City_Category == 'B',1,0)
X_test <- X_test[,-c(1,7)]
...
model_xgb_1 <- XGBoost(X_train,y,X_test,cv=5,objective="reg:linear",
                       nrounds=500,max.depth=10,eta=0.1,colsample_bytree=0.5,
                       seed=235,metric="rmse",importance=1)


model_xgb_2 <- XGBoost(X_train,y,X_test,cv=5,objective="reg:linear",
                       nrounds=500,max.depth=10,eta=0.1,colsample_bytree=0.5,
                       seed=235,metric="rmse",importance=1)

## submission file
test_xgb_1 <- model_xgb_1[[2]]
train_xgb_1 <- model_xgb_1[[1]]
...
#submit$Purchase[submit$Purchase < 600] <- 600
#submit$Purchase[submit$Purchase > 24000] <- 23900

#write.csv(train_xgb_1, "./train_xgb.csv", row.names=F)

write.csv(submit, "./submit_6.csv", row.names=F)
